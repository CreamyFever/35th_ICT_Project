...
public class BoardDAO {
	 ...
	 * @return 글목록
	 */
	/*public ArrayList<Board> subjectListBoard(String subject, int startRecord, int countPerPage, String searchText) {
		BoardMapper mapper = sqlSession.getMapper(BoardMapper.class);
		//전체 검색 결과 중 읽을 시작위치와 개수
		RowBounds rb = new RowBounds(startRecord, countPerPage);
		
		//검색어와 읽을 범위를 전달
		ArrayList<Board> boardlist = mapper.subjectListBoard(subject,searchText, rb);
		return boardlist;
	}*/

	public ArrayList<Board> subjectListBoard(String subject, int startRecord, int countPerPage) {
		BoardMapper mapper = sqlSession.getMapper(BoardMapper.class);
		//전체 검색 결과 중 읽을 시작위치와 개수
		RowBounds rb = new RowBounds(startRecord, countPerPage);
		
		ArrayList<Board>boardlist = mapper.subjectListBoard(subject,rb);
		return boardlist;
